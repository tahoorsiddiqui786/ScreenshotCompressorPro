...
export default function Home() {
  ...
  const TARGET_SIZE = 201 * 1024;
  const LOGO_URL = '/logo.png'; // Your Y&Z logo

  const addLogoToCanvas = async (canvas) => {
    return new Promise((resolve) => {
      ...
      img.onload = () => {
        ...
        // Logo size - small bottom right corner
        const logoSize = Math.min(canvas.width, canvas.height) * 0.08; // 8% of smallest dimension
        const padding = 20;
        ...
        ctx.roundRect(x - 10, y - 10, logoSize + 20, logoSize + 20, 10);
        ctx.fill();
  ...
  const compressImage = async (canvas, addLogo = true) => {
    setCompressionStatus('Adding watermark...');
    
    // Add logo to canvas
    if (addLogo) {
      canvas = await addLogoToCanvas(canvas);
    }
